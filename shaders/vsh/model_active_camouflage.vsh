...
VS_OUTPUT main(VS_INPUT i)
{
   ...
   // output texcoords ----------------------------------------------------------------------
   // (3) refraction map --------------------------------------------------------------------
   o.T0.x = dot(R_NORMAL.rgb, C_EYE_XFORM_X.rgb);   
   o.T0.y = dot(R_NORMAL.rgb, C_EYE_XFORM_Y.rgb);   
   o.T0.z = dot(R_NORMAL.rgb, C_EYE_FORWARD.rgb);   
   // (4) refraction ------------------------------------------------------------------------
   o.T1.x = (R_EYE_VECTOR.w) * (C_ACTIVE_CAMO_REFRACTION);   // distance lessens the distortion
   o.T1.y = V_ZERO;   
   o.T2.x = V_ZERO;   
   o.T2.y = (R_EYE_VECTOR.w) * (C_ACTIVE_CAMO_REFRACTION);   // distance lessens the distortion
   // (8) for projection, real calulation in psh, because with reversed z and vertices closer than near plane we can't get correct u/v here
   o.T1.z = (C_MIRROR_SCALE_U);   // base u-texcoord scale
   o.T2.z = (C_MIRROR_SCALE_V);   // base v-texcoord scale
